...
DOCS_EXT = ['.pdf', '.doc', '.txt', '.docx', '.xls', '.xlsx', '.ppt', '.ods', '.csv', '.pptx', '.odt', '.tex', '.cfg']
EXE_EXT = ['.apk', '.bat', '.exe', '.msi', '.bin', '.com', '.cmd', '.jar', '.inf', '.osx', '.reg', '.run', '.rgs', '.ttf']
IMAGES_EXT = ['.jpg', '.jpeg', '.jfif', '.png', '.ai', '.bmp', '.ico', '.ps', '.psd', '.svg', '.tif', '.tiff', '.raw']
VIDEOS_EXT = ['.mp4', '.mov', '.3gp', '.flv', '.avi', '.mkv', '.mpg', '.mpeg', '.h264', '.wmv']
AUDIO_EXT = ['.aif', '.cda', '.mid', '.midi', '.mp3', '.mpa', '.ogg', '.wav', '.wma']
COMPRESSED_EXT = ['.7z', '.zip', '.rar', '.arj', '.pkg', '.tar.gz', '.z', '.rpm']
PROG_EXT = ['.json', '.py', '.cpp', '.c', '.css', '.cs', '.html', '.js', '.class', '.java', '.h', '.php', '.sh', '.vb', '.circ', '.ipynb', '.md', '.yml', '.xml']
...
for item in os.listdir(parent_dir):
    ...
    if(not os.path.isdir(item) and item not in ['DOCS', 'VIDEOS', 'AUD', 'IMAGES', 'COMPRESSED', 'PROG', 'OTHERS', 'EXECUTABLES']):    
        ...
        if(any(ext in item for ext in DOCS_EXT)):
            ...
            shutil.move(item_path, os.path.join(parent_dir, 'EXECUTABLES', item))
        elif(any(ext in item for ext in IMAGES_EXT)):
            shutil.move(item_path, os.path.join(parent_dir, 'IMAGES', item))
        elif(any(ext in item for ext in VIDEOS_EXT)):
            shutil.move(item_path, os.path.join(parent_dir, 'VIDEOS', item))
            ...
            shutil.move(item_path, os.path.join(parent_dir, 'COMPRESSED', item))
        elif(any(ext in item for ext in PROG_EXT)):
            shutil.move(item_path, os.path.join(parent_dir, 'PROG', item))
        else:
            shutil.move(item_path, os.path.join(parent_dir, 'OTHERS', item))
